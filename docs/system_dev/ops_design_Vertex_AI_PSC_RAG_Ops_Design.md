...
| 指標 | 目標値 | 測定区間 | 備考 |
| :--- | :--- | :--- | :--- |
| RAG検索・生成レイテンシ | 5.0s 以内 (p95) | API 受信 〜 レスポンス送信 | 全体の一括回答生成 |
| API 稼働率 | 99.9% | 月間合計時間 | |
...
- **Cloud Monitoring**:
    ...
    - `discoveryengine.googleapis.com/request_count`: Vertex AI Search のクォータ超過 (429) を監視。
- **アラート条件**:
...
## 5. インシデント対応
- **切り分けフロー**:
    1. ネットワークエラー: PSC 接続エラー確認 (Network Intelligence Center)
    2. サービスダウン: Vertex AI Search サービス稼働状況確認 (Google Cloud Status Dashboard)
    3. 連携エラー: Application Integration フローの実行ログ確認 (Visualizer)
    4. クォータエラー: Vertex AI Search 429 発生時のクォータ引き上げ検討。
