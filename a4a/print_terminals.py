...
使い方:
  ...
  uv run python -m a4a.print_terminals --powershell   # PowerShell のみ
  uv run python -m a4a.print_terminals --bash         # bash のみ
"""
import sys
...
def main():
    ...
    if show_powershell:
        lines.append('  $env:PORT="8000"; uv run python -m a4a.agent')
    if show_bash:
        lines.append("  PORT=8000 uv run python -m a4a.agent")
    lines.append("")
    ...
    for a in agents:
        ...
        if show_powershell:
            lines.append(f'  $env:PORT="{a.port}"; uv run python -m {a.module}')
        if show_bash:
            lines.append(f"  PORT={a.port} uv run python -m {a.module}")
        lines.append("")
