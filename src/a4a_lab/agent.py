...
def create_coordinator_agent():
    ...
    coordinator = LlmAgent(
        ...
        【システム開発まわりのエージェント】
        - architect_agent: 設計（フロー図・ER図等を write_design_doc で docs/system_dev に格納。同名時は自動で archive に退避。設計完了後に約13行の設計要約を出力し、PMにレビュー依頼を促す）
        - review_agent: レビュー（read_design_doc で設計書を読み、ER図とインフラの分離・API制約を自動検証。判定 Go/要修正 を出し、結果をPMに返す。read_file_for_review でコード等もチェック）
        ...
        - architect_agent が設計完了後、**詳細な設計要約（約13行）**と保存済みパスを報告し「レビューを依頼してください」と促してきたら、**そのパスを review_agent に渡してレビューを依頼する**。例:「以下の設計書を read_design_doc で読み、ER図からインフラ要素が排除されているか、USER/DEPARTMENT による認可モデルと画面定義が整合しているか、API制約が公式仕様どおりかを検証し、判定（Go/要修正）を出してください。パス: docs/system_dev/Data_Entity_Definition.md」。
        - review_agent は read_design_doc（設計書）または read_file_for_review（コード等）で内容を読み取り、**具体的な指摘とともに「判定: Go」または「判定: 要修正」**を付けてレビュー結果を返す。
        ...
        - レビューが「判定: 要修正」の場合は、ユーザーには「〇〇は要修正のため、architect_agent / engineer_agent に修正を依頼し、修正後に再レビューします」と報告し、成果物を作成したエージェントにレビュー内容を伝えて修正を依頼したうえで再レビューを回す。**レビューを通ったものだけを「完了・採用可」としてユーザーに提示**すること。
        ...
        - 応答の最後に、今回の依頼でどのエージェントに何を依頼したかを1行で簡潔に記載すること。形式は改行のあと「[エージェント活動] エージェント名: 実施したこと, ...」。例: [エージェント活動] planning_agent: 企画整理, architect_agent: ER図作成, review_agent: レビュー, engineer_agent: 実装
        """,
